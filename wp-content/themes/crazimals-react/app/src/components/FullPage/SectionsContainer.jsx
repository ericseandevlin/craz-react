...
export default class SectionsContainer extends React.Component {
  ...
  constructor(props) {
    ...
    this.nodesLoadedToCanvas = 0
    this.workedOnParentOnceAfterAllChildrenWereLoaded = false
  }
  ...
  componentDidMount() {
    console.log('SectionsContainer did mount');
    this._childrenLength = this.props.children && this.props.children.length;
  ...
  _addMouseWheelEventHandlers() {
    window.addEventListener('mousewheel', this._handleMouseWheel, {passive: true});
    window.addEventListener('DOMMouseScroll', this._handleMouseWheel, {passive: true});
  }
